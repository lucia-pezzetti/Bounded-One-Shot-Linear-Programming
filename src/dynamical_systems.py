...
class point_mass_cubic_drag_2du(point_mass_cubic_drag):
    ...
    Underactuation:
        u ∈ R^2 always. You can choose a dense random B (benchmark default)
        or a sparse structured B (endpoint actuation); coupling through K
    ...
    def __init__(
        ...
        # ---- Choose B (input map) ----
        # If the user supplies B explicitly, we honor it.
        # Otherwise, we provide two benchmark-friendly options:
        #
        #   B_mode = "dense_random" (default):
        #       Dense random actuation directions (good for control benchmarks).
        #   B_mode = "sparse_structured":
        #       Actuate two physical coordinates (first and last) — the previous default.
        #
        # B_scale controls the overall actuation strength (after optional column-normalization).
        if B is None:
            if rng is None:
            ...
            if mode in ["dense_random", "dense", "random", "rand"]:
                B = rng.standard_normal((self.n, 2))
                # Normalize columns so each input has comparable strength
                col_norms = np.linalg.norm(B, axis=0, keepdims=True)
                col_norms = np.maximum(col_norms, 1e-12)
                B = B / col_norms
                B = float(B_scale) * B
            elif mode in ["sparse_structured", "sparse", "structured", "endpoints", "endpoint"]:
